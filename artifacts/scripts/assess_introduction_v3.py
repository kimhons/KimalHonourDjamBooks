...
# Define file paths
input_file = '/home/ubuntu/book_assessment/chapter_enhancements/introduction_enhanced_v3.md'
...
def analyze_quality(content, title):
    ...
    if title and len(re.findall(r'^##\s+(.*?)$', content, re.MULTILINE)) >= 5:
        score += 15
    elif title and len(re.findall(r'^##\s+(.*?)$', content, re.MULTILINE)) >= 3:
        score += 10
    ...
    if practical_count >= 15:
        score += 20
    elif practical_count >= 10:
        score += 15
