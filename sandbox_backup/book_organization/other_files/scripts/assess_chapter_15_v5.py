...
# Configuration
CHAPTER_FILE = "/home/ubuntu/book_assessment/chapter_enhancements/chapter_15_enhanced_v5.md"
OUTPUT_JSON = "/home/ubuntu/book_assessment/chapter_assessments/chapter_15_final_assessment.json"
...
def calculate_seo_score(content, title):
    ...
    if len(re.findall(r"^###\s+", content, re.MULTILINE)) >= 5: score += 2 # More subsections for framework
    if len(re.findall(r"^\*\s+", content, re.MULTILINE)) >= 15: score += 5 # Expecting more list items for principles/practices
...
def calculate_quality_score(content, word_count):
    ...
    # Depth and detail (proxy by word count - adjust expectation based on current count)
    if word_count > 5000: score += 2
    if word_count > 6000: score += 1 # Lowered threshold slightly due to current count

    # Structure and clarity
    if len(re.findall(r"^##\s+", content, re.MULTILINE)) >= 3: score += 2
    if len(re.findall(r"^###\s+", content, re.MULTILINE)) >= 5: score += 2
