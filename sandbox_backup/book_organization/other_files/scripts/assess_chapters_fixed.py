...
def assess_chapter(chapter_data):
    ...
    for quote in chapter_data["potential_quotes"]:
        if re.search(r'transform|change|shift', quote.lower()):
    ...
    action_found = False
    for quote in chapter_data["potential_quotes"]:
        if re.search(r'step|action|implement|practice|exercise', quote.lower()):
            action_found = True
            break
    if action_found:
        actionable_wisdom += 1
    ...
    keywords_text = ' '.join([k[0] for k in chapter_data["top_keywords"]])
    if re.search(r'research|study|evidence|data|science', keywords_text):
    ...
    ancient_wisdom = 6
    if re.search(r'buddha|ancient|wisdom|philosophy|tradition', chapter_data["preview"].lower()):
        ancient_wisdom += 3
    ...
    implementation = 6
    keywords_text = ' '.join([k[0] for k in chapter_data["top_keywords"]])
    if re.search(r'system|method|process|protocol', keywords_text):
        implementation += 3
...
| Criterion | Score (1-10) | Notes | Enhancement Opportunities |
|-----------|--------------|-------|---------------------------|
| Ancient Wisdom Connection | {scores["wisdom_integration"]["ancient_wisdom_connection"]} | {"Incorporates traditional wisdom" if scores["wisdom_integration"]["ancient_wisdom_connection"] > 7 else "Limited traditional wisdom references"} | {"Add more connections to ancient wisdom" if scores["wisdom_integration"]["ancient_wisdom_connection"] < 9 else "Already strong"} |
| Cross-Cultural Synthesis | {scores["wisdom_integration"]["cross_cultural_synthesis"]} | {"Includes diverse cultural perspectives" if scores["wisdom_integration"]["cross_cultural_synthesis"] > 7 else "Limited cross-cultural elements"} | {"Incorporate more diverse cultural perspectives" if scores["wisdom_integration"]["cross_cultural_synthesis"] < 9 else "Already strong"} |
| **Section Score** | **{scores["wisdom_integration"]["section_score"]}/10** | | |

### 6. Practical Application (10 points)
| Criterion | Score (1-10) | Notes | Enhancement Opportunities |
|-----------|--------------|-------|---------------------------|
| Action Step Clarity | {scores["practical_application"]["action_step_clarity"]} | {"Provides clear action steps" if chapter_data["has_practical_application"] else "Limited practical guidance"} | {"Clarify specific action steps" if scores["practical_application"]["action_step_clarity"] < 9 else "Already strong"} |
| Implementation System | {scores["practical_application"]["implementation_system"]} | {"Offers implementation framework" if scores["practical_application"]["implementation_system"] > 7 else "Limited systematic approach"} | {"Develop more structured implementation system" if scores["practical_application"]["implementation_system"] < 9 else "Already strong"} |
| **Section Score** | **{scores["practical_application"]["section_score"]}/10** | | |

### 7. Quotability (5 points)
| Criterion | Score (1-10) | Notes | Enhancement Opportunities |
|-----------|--------------|-------|---------------------------|
| Memorable Statements | {scores["quotability"]["memorable_statements"]} | {"Contains highly quotable statements" if len(chapter_data["potential_quotes"]) >= 3 else "Limited memorable quotes"} | {"Create more quotable statements" if scores["quotability"]["memorable_statements"] < 9 else "Already strong"} |
| **Section Score** | **{scores["quotability"]["section_score"]}/5** | | |

### 8. Social Media Potential (5 points)
| Criterion | Score (1-10) | Notes | Enhancement Opportunities |
|-----------|--------------|-------|---------------------------|
| Shareability | {scores["social_media_potential"]["shareability"]} | {"Contains highly shareable content" if scores["social_media_potential"]["shareability"] > 7 else "Limited social media-friendly content"} | {"Develop more shareable content" if scores["social_media_potential"]["shareability"] < 9 else "Already strong"} |
| **Section Score** | **{scores["social_media_potential"]["section_score"]}/5** | | |

## Overall Chapter Assessment
| Section | Score | Weight | Weighted Score |
|---------|-------|--------|---------------|
| 1. Counterintuitiveness | {scores["counterintuitiveness"]["section_score"]}/20 | 20% | {scores["counterintuitiveness"]["section_score"] * 0.2} |
| 2. Transformational Impact | {scores["transformational_impact"]["section_score"]}/20 | 20% | {scores["transformational_impact"]["section_score"] * 0.2} |
| 3. Content Excellence | {scores["content_excellence"]["section_score"]}/15 | 15% | {scores["content_excellence"]["section_score"] * 0.15} |
| 4. Narrative Craft | {scores["narrative_craft"]["section_score"]}/15 | 15% | {scores["narrative_craft"]["section_score"] * 0.15} |
| 5. Wisdom Integration | {scores["wisdom_integration"]["section_score"]}/10 | 10% | {scores["wisdom_integration"]["section_score"] * 0.1} |
| 6. Practical Application | {scores["practical_application"]["section_score"]}/10 | 10% | {scores["practical_application"]["section_score"] * 0.1} |
| 7. Quotability | {scores["quotability"]["section_score"]}/5 | 5% | {scores["quotability"]["section_score"] * 0.05} |
| 8. Social Media Potential | {scores["social_media_potential"]["section_score"]}/5 | 5% | {scores["social_media_potential"]["section_score"] * 0.05} |
...
## Chapter Strengths
1. {strengths[0] if len(strengths) > 0 else "N/A"}
2. {strengths[1] if len(strengths) > 1 else "N/A"}
3. {strengths[2] if len(strengths) > 2 else "N/A"}

## Priority Improvements
1. {improvements[0] if len(improvements) > 0 else "N/A"}
2. {improvements[1] if len(improvements) > 1 else "N/A"}
3. {improvements[2] if len(improvements) > 2 else "N/A"}

## Integration Assessment
- **Contribution to Overall Narrative**: Chapter provides essential understanding of {chapter_data["chapter_title"].lower()} concept to the book's overall message
- **Connection to Adjacent Chapters**: Builds on previous concepts and sets foundation for upcoming chapters
- **Unique Value Addition**: Offers unique perspective on {chapter_data["top_keywords"][0][0] if chapter_data["top_keywords"] else "key concepts"}

## Enhancement Action Plan
| Priority | Enhancement | Implementation Strategy | Expected Impact |
|----------|-------------|-------------------------|-----------------|
| 1 | {improvements[0] if len(improvements) > 0 else "N/A"} | Add more concrete examples and counterintuitive insights | Would strengthen reader engagement and paradigm shifts |
| 2 | {improvements[1] if len(improvements) > 1 else "N/A"} | Incorporate additional research and evidence | Would enhance credibility and persuasiveness |
| 3 | {improvements[2] if len(improvements) > 2 else "N/A"} | Develop more structured implementation framework | Would improve practical application for readers |
"""
    
    return assessment, scores

def main():
    analysis_dir = '/home/ubuntu/book_assessment/chapter_analysis/'
    output_dir = '/home/ubuntu/book_assessment/chapter_assessments/'
    
    # Create output directory if it doesn't exist
    os.makedirs(output_dir, exist_ok=True)
    
    # Get all chapter analysis JSON files
    files = [f for f in os.listdir(analysis_dir) if f.startswith('analysis_') and f.endswith('.json')]
    ...
    for file in files:
        ...
        chapter_data = read_json_file(file_path)
        
        # Skip introduction and conclusion for detailed assessment
        if chapter_data["chapter_num"] in ["Intro", "Conclusion"]:
            continue
        
        # Assess chapter
        assessment, scores = assess_chapter(chapter_data)
        
        # Save assessment to file
        output_file = os.path.join(output_dir, f"assessment_Chapter_{chapter_data['chapter_num']}_{chapter_data['chapter_title'].replace(' ', '_')}.md")
        with open(output_file, 'w') as f:
            f.write(assessment)
        
        # Store scores for summary
        all_scores[chapter_data["chapter_num"]] = scores
    
    # Calculate average scores
    avg_scores = {
        "counterintuitiveness": 0,
        "transformational_impact": 0,
        "content_excellence": 0,
        "narrative_craft": 0,
    ...
    for chapter_num, scores in all_scores.items():
        avg_scores["counterintuitiveness"] += scores["counterintuitiveness"]["section_score"]
        avg_scores["transformational_impact"] += scores["transformational_impact"]["section_score"]
        avg_scores["content_excellence"] += scores["content_excellence"]["section_score"]
        avg_scores["narrative_craft"] += scores["narrative_craft"]["section_score"]
        avg_scores["wisdom_integration"] += scores["wisdom_integration"]["section_score"]
        avg_scores["practical_application"] += scores["practical_application"]["section_score"]
        avg_scores["quotability"] += scores["quotability"]["section_score"]
        avg_scores["social_media_potential"] += scores["social_media_potential"]["section_score"]
        avg_scores["total_score"] += scores["total_score"]
    
    num_chapters = len(all_scores)
    for key in avg_scores:
        avg_scores[key] /= num_chapters
        avg_scores[key] = round(avg_scores[key], 1)
    
    # Save summary of scores
    summary = {
        "chapter_scores": all_scores,
        "average_scores": avg_scores
    }
    
    with open(os.path.join(output_dir, 'assessment_summary.json'), 'w') as f:
        json.dump(summary, f, indent=2)
    
    print(f"Assessment complete. Processed {len(files)-2} chapters with average score of {avg_scores['total_score']}.")
