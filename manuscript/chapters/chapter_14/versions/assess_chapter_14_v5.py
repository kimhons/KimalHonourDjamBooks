...
# Configuration
CHAPTER_FILE = "/home/ubuntu/book_assessment/chapter_enhancements/chapter_14_enhanced_v5.md"
OUTPUT_JSON = "/home/ubuntu/book_assessment/chapter_assessments/chapter_14_final_assessment.json"
...
def calculate_seo_score(content, title):
    ...
    score = 75 # Base score
    keywords = ["mindset shift", "unbothered mindset", "control mindset", "strategic detachment", 
                "neuroscience", "resilience", "stress", "effectiveness", "presence", "perspective", 
                "cognitive reappraisal", "neuroplasticity", "brain", "allostatic load"]
    content_lower = content.lower()
    title_lower = title.lower()

    # Keyword in title
    if any(kw in title_lower for kw in ["mindset shift", "unbothered", "control mindset"]):
        score += 5
    ...
    if keyword_mentions > 150: score += 5

    # Structure (headings, lists)
    if len(re.findall(r"^##\s+", content, re.MULTILINE)) >= 3: score += 3
    if len(re.findall(r"^###\s+", content, re.MULTILINE)) >= 3: score += 2
    if len(re.findall(r"^\*\s+", content, re.MULTILINE)) >= 10: score += 5

    # Blockquotes for highlighted content
    if len(re.findall(r"^>", content, re.MULTILINE)) >= 5: score += 5
...
def calculate_quality_score(content, word_count):
    ...
    if "Ultimate Mindset Shift Framework" in content: score += 3
    
    # Narrative elements
    if "Rachel" in content and "Dr. Wilson" in content: score += 3
    
    # Counterintuitive insights
    if "paradox" in content.lower() and "counterintuitive" in content.lower(): score += 3
    
    # Practical application
    if "Key Practices" in content: score += 3
